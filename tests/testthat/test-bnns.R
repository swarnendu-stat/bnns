...
test_that("bnns throws appropriate error for missing formula or data", {
  ...
  expect_error(bnns(y ~ -1 + x1 + x2, out_act_fn = 1, iter = 1e2, warmup = 5e1, chains = 1),
               "Both 'formula' and 'data' must be provided.")

  expect_error(bnns(data = df, out_act_fn = 1, iter = 1e2, warmup = 5e1, chains = 1),
               "Both 'formula' and 'data' must be provided.")
})
...
test_that("bnns.default throws errors for incorrect inputs", {
  ...
  expect_error(bnns(y ~ -1 + x1 + x2, data = df, out_act_fn = 2, iter = 1e2, warmup = 5e1, chains = 1),
