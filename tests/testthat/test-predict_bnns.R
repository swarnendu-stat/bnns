...
model_f <- bnns(y ~ -1 + x1 + x2, data = data, L = 1, nodes = 2)
model_nf <- bnns(with(data, cbind(x1, x2)), data$y, L = 1, nodes = 2)
...
test_that("predict.bnns handles formula and non-formula cases", {
  ...
  expect_equal(apply(formula_predictions, 1, mean), apply(matrix_predictions, 1, mean))
})
